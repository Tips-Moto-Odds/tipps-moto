<script setup>
import SubscribeForm from "@/Pages/Users/Tips/Componsnets/SubscribeForm.vue"
import UnSubscribeForm from "@/Pages/Users/Tips/Componsnets/UnSubscribeForm.vue"
...
const time_remaining = (user_data) => {
    let error;
    try {
        if (!user_data?.start_date || !user_data?.end_date) {
            error =  new Error("Missing start_date or end_date in subscriptions_details")
            throw error;
        }

        const start = new Date(user_data.start_date)
        const end = new Date(user_data.end_date)
...
<template>
    <div class="flex gap-3 px-[10px] pan w-full">
        <div class="app-panel w-full">
            ...
            <ul class="text-sm text-white m-[15px]">
                ...
                <li>
                    <p>Status :</p>
                    <p>{{ user_data?.status ?? 'N/A' }}</p>
                </li>
            </ul>
            <div class="md:w-[300px] md:float-end">
                <button v-if="user_data" class="bg-red-400" @click.prevent.stop="unSubscribe">Unsubscribe</button>
